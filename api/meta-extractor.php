...
$html=curl_exec($ch);$err=curl_error($ch);$status=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);$contentType=(string)curl_getinfo($ch,CURLINFO_CONTENT_TYPE);$finalUrl=(string)curl_getinfo($ch,CURLINFO_EFFECTIVE_URL);$bytes=strlen((string)$html);curl_close($ch);
...
$meta=function(string $key,string $attr):string use($xp):string{$n=$xp->query('//meta[translate(@'.$attr.',"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz")="'.htmlspecialchars(strtolower($key),ENT_QUOTES).'" ]');return($n&&$n->length)?trim((string)$n->item(0)->getAttribute('content')):'';};
$title=$first('//title');$description=$meta('description','name');$robots=$meta('robots','name');$lang=(string)$dom->documentElement? (string)$dom->documentElement->getAttribute('lang'):'';$viewport=$meta('viewport','name');$canonical=$first('//link[contains(translate(@rel,"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz"),"canonical")]/@href');$h1=$xp->query('//h1');$h1Count=$h1?$h1->length:0;
...
echo json_encode(['ok'=>true,'status'=>$status,'statusText'=>trim((string)http_response_code()),'contentType'=>$contentType,'bytes'=>$bytes,'finalUrl'=>$finalUrl,'title'=>$title,'description'=>$description,'robots'=>$robots,'lang'=>$lang,'viewport'=>$viewport,'canonical'=>$canonical,'h1Count'=>$h1Count,'headings'=>$headings,'social'=>$social,'ogCount'=>$ogCount,'twitterCount'=>$twitterCount],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
