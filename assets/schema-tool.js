...
(function(){
  ...
  function buildSchema(v){
    ...
    if(cfg.schemaType==='BreadcrumbList'){
      s.itemListElement=(cfg.counts||6).map((_,i)=>({"@type":"ListItem",position:i+1,name:v['n'+i],item:cleanUrl(v['u'+i])})).filter(x=>x.name&&x.item);
    }
